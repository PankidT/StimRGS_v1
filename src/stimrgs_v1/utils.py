...
def fast_generate_combinations_with_adjustable_replacement(
    nodes: list, max_length: int, max_self_combination: int
) -> list:
    """
...
def find_neighbors_from_selected_node(input_list: list, selected_node: int) -> list:
    ...
    grouped_elements = [
        (input_list[i], input_list[i + 1]) for i in range(0, len(input_list) - 1, 2)
    ]
...
def generate_h_cz_string(numbers: list, num_nodes: int, fix_nodes: bool = False) -> str:
    if fix_nodes:
...
def local_complementation_stim(
    index: int, queries: list, circuit: str
) -> tuple[stim.TableauSimulator, list[str], str]:
